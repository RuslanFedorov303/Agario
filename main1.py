...
player_img = image.load('images/ТРАМП_КРУГ.png')
enemy_img = image.load('images/МАДУРО_КРУГ.png')
bg_img = image.load('images/fon.png')
...
while running:
    ...
    scaled_bg = transform.scale(bg_scaled_size, (bg_scaled_size, bg_scaled_size))

    base_bg_x = int((-2000 - ball.x) * ball.scale + size[0] // 2)
    base_bg_x = int((-2000 - ball.y) * ball.scale + size[1] // 2)

    start_x = (bace_bg_x % bg_scaled_size) - bg_scaled_size
    start_y = (bace_bg_y % bg_scaled_size) - bg_scaled_size
